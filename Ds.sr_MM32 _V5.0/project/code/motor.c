...
/************************电机保护*********************/
	...
	Motor_Pid.Dif_Speed=Incremental_PI (Track.Err,0);
...
{
	int8 Dir_last_error,Dir_error;
	int8 Dif_Speed;
	Dir_error=Target-err;                
	Dif_Speed+=Motor_Pid.Dir_Kp*(Dir_error-Dir_last_error)+Motor_Pid.Dir_Ki*Dir_error;   //使用增量 PI 控制器求出电机 PWM。
